...
if not st.session_state.soldier_connected:
    ...
    with right_panel:
               ...
               (msg['sender_id'] == DEVICE_ID and msg['receiver_id'] == COUNTERPART_ID):
                with chat_container.chat_message("user" if msg["sender_id"] == DEVICE_ID else "assistant"):
                    if msg.get('type') == 'file':
                        ...
                        if 'image' in mimetype: st.write(f"Image Recon Received: `{msg['filename']}`"); st.image(f"{BACKEND_URL}/download/{msg['id']}")
                        elif 'video' in mimetype: st.write(f"Video Briefing Received: `{msg['filename']}`"); st.video(f"{BACKEND_URL}/download/{msg['id']}")
        ...
        uploaded_file = st.file_uploader("Send Secure File", type=['png', 'jpg', 'jpeg', 'mp4', 'mov'], key="soldier_uploader")
        if uploaded_file:
            if st.button("Transmit File"):
