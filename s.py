...
st.markdown(
    ...
    .stTextInputdivdivinput, .stButtonbutton, .stChatInputdivdivinput {
        background-color #001a00 !important; color #00FF41 !important;
        border 1px solid #00FF41 !important;
    }
    .stButtonbuttonhover { background-color #003300 !important; }
    ...
    div[data-testid=stChatMessage] buttonhover { background-color #660000 !important; }
        ...
        for msg in st.session_state.message_history
            with chat_container.chat_message(user if msg[sender_id] == DEVICE_ID else assistant)
                ...
                if msg[status] == 'recalled'
                    msg_col.markdown(fp class='recalled-message'{msg['message']}p, unsafe_allow_html=True)
                else
                    msg_col.write(msg[message])
                
                if msg[sender_id] == DEVICE_ID and msg[status] != 'recalled'
                    # ## UPDATED BUTTON TEXT ##
                    if button_col.button(R, key=fdel_{msg['id']}, help=Recall message)
